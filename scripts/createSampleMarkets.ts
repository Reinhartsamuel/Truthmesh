// scripts/createSampleMarkets.ts
...
async function createSampleMarkets() {
  ...
  try {
    ...
    try {
      ...
      if (code === '0x') {
        console.log(`❌ No contract code at address ${process.env.PREDICTION_MARKET_ADDRESS}!`);
        console.log(`   The contract may not be deployed or the address is incorrect.`);
        console.log(`   Latest deployment on BSC Testnet (chain 97): 0xa2cca5a07bbed0d25543e7ef1e8a0b29e8bd4a6e`);
        console.log(`   Please update PREDICTION_MARKET_ADDRESS environment variable.`);
        process.exit(1);
      }
